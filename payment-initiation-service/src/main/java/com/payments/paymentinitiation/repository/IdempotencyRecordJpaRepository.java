 ...
 * 
 * Provides data access methods for idempotency tracking
 */
@Repository
public interface IdempotencyRecordJpaRepository extends JpaRepository<IdempotencyRecordEntity, String> {
     ...
     */
    @Query("SELECT COUNT(i) > 0 FROM IdempotencyRecordEntity i WHERE i.idempotencyKey = :idempotencyKey AND i.tenantId = :tenantId")
    boolean existsByIdempotencyKeyAndTenantId(@Param("idempotencyKey") String idempotencyKey, @Param("tenantId") String tenantId);
     ...
     */
    @Query("SELECT i FROM IdempotencyRecordEntity i WHERE i.idempotencyKey = :idempotencyKey AND i.tenantId = :tenantId")
    Optional<IdempotencyRecordEntity> findByIdempotencyKeyAndTenantId(@Param("idempotencyKey") String idempotencyKey, @Param("tenantId") String tenantId);
     ...
     */
    @Query("SELECT i FROM IdempotencyRecordEntity i WHERE i.tenantId = :tenantId ORDER BY i.createdAt DESC")
