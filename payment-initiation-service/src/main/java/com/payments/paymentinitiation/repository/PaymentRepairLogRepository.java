 ...
 *
 * <p>Repository for payment repair log entities. Provides queries for repair history and audit
 * trails.
 */
@Repository
public interface PaymentRepairLogRepository extends JpaRepository<PaymentRepairLogEntity, Long> {
  ...
  List<PaymentRepairLogEntity> findByActionOrderByTimestampDesc(
      PaymentRepairLogEntity.RepairAction action);

  /** Find repair logs by time range */
  @Query(
      "SELECT r FROM PaymentRepairLogEntity r WHERE r.timestamp BETWEEN :startTime AND :endTime ORDER BY r.timestamp DESC")
  ...
  @Query(
      "SELECT r FROM PaymentRepairLogEntity r WHERE r.paymentId = :paymentId AND r.timestamp BETWEEN :startTime AND :endTime ORDER BY r.timestamp DESC")
