 ...
 *
 * <p>Maps to the payments table with proper JPA annotations and multi-tenancy support
 */
@Entity
@Table(
    name = "payments",
...
public class PaymentEntity {
  ...
  @AttributeOverrides({
    @AttributeOverride(
        name = "tenantId",
        column = @Column(name = "tenant_id", nullable = false, length = 20)),
    @AttributeOverride(
        name = "businessUnitId",
        column = @Column(name = "business_unit_id", nullable = false, length = 30))
  })
  ...
  public void addStatusChange(PaymentStatus newStatus, String reason) {
    PaymentStatusHistoryEntity historyEntry =
