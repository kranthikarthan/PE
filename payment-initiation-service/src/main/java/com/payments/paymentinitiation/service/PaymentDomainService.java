 ...
 *
 * <p>Handles complex business logic that doesn't belong to a single aggregate: - Business rule
 * validation - Cross-aggregate operations - Domain event coordination
 */
@Slf4j
@Service
...
public class PaymentDomainService {
  ...
  private void validateDailyLimits(Payment payment, TenantContext tenantContext) {
    ...
    if (newTotal.compareTo(dailyLimit) > 0) {
      throw new IllegalArgumentException(
          String.format(
              "Daily payment limit exceeded. Current: %s, Limit: %s", newTotal, dailyLimit));
    }
  ...
  private void validateVelocityLimits(Payment payment, TenantContext tenantContext) {
    ...
    if (paymentCount >= velocityLimit) {
      throw new IllegalArgumentException(
          String.format(
              "Payment velocity limit exceeded. Count: %d, Limit: %d",
  ...
  public void processPaymentStatusChange(Payment payment, PaymentStatus newStatus, String reason) {
    log.info(
        "Processing status change for payment: {} from {} to {}",
        payment.getId(),
        payment.getStatus(),
        newStatus);
    ...
    boolean isValidTransition =
        switch (currentStatus) {
          ...
          case SUBMITTED_TO_CLEARING -> newStatus == PaymentStatus.CLEARING
              || newStatus == PaymentStatus.FAILED;
          case CLEARING -> newStatus == PaymentStatus.CLEARED || newStatus == PaymentStatus.FAILED;
          case CLEARED -> newStatus == PaymentStatus.COMPLETED || newStatus == PaymentStatus.FAILED;
