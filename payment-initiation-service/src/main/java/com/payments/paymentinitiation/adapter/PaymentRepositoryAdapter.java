 ...
 * 
 * Implements the PaymentRepositoryPort using JPA repository
...
public class PaymentRepositoryAdapter implements PaymentRepositoryPort {
    ...
    public Optional<Payment> findByIdAndTenantId(PaymentId paymentId, String tenantId) {
        ...
        return jpaRepository.findByIdAndTenantId(paymentId, tenantId)
                .map(paymentMapper::toDomain);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<Payment> findByTenantIdAndBusinessUnitId(String tenantId, String businessUnitId, Pageable pageable) {
        log.debug("Finding payments by tenant and business unit: {}, {}", tenantId, businessUnitId);
        
        return jpaRepository.findByTenantIdAndBusinessUnitId(tenantId, businessUnitId, pageable)
                .map(paymentMapper::toDomain);
    }
    ...
    public List<Payment> findByStatusAndTenantId(PaymentStatus status, String tenantId) {
        ...
        return jpaRepository.findByStatusAndTenantId(status, tenantId)
                .stream()
                .map(paymentMapper::toDomain)
                .toList();
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<Payment> findByTenantIdAndDateRange(String tenantId, Instant startDate, Instant endDate, Pageable pageable) {
        log.debug("Finding payments by tenant and date range: {}, {} to {}", tenantId, startDate, endDate);
        
        return jpaRepository.findByTenantIdAndDateRange(tenantId, startDate, endDate, pageable)
                .map(paymentMapper::toDomain);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<Payment> findBySourceAccountAndTenantId(String sourceAccount, String tenantId, Pageable pageable) {
        log.debug("Finding payments by source account and tenant: {}, {}", sourceAccount, tenantId);
        
        return jpaRepository.findBySourceAccountAndTenantId(sourceAccount, tenantId, pageable)
                .map(paymentMapper::toDomain);
    }

    @Override
    @Transactional(readOnly = true)
    public Page<Payment> findByDestinationAccountAndTenantId(String destinationAccount, String tenantId, Pageable pageable) {
        log.debug("Finding payments by destination account and tenant: {}, {}", destinationAccount, tenantId);
        
        return jpaRepository.findByDestinationAccountAndTenantId(destinationAccount, tenantId, pageable)
                .map(paymentMapper::toDomain);
    }
