 ...
 *
 * <p>Implements the IdempotencyRepositoryPort using JPA repository following the Ports and Adapters
 * pattern
 */
@Slf4j
@Component
...
public class IdempotencyRepositoryAdapter implements IdempotencyRepositoryPort {
  ...
  @Override
  @Transactional(readOnly = true)
  public Optional<IdempotencyRecord> findByIdempotencyKeyAndTenantId(
  ...
  public List<IdempotencyRecord> findByPaymentId(String paymentId) {
    ...
    return jpaRepository.findByPaymentId(paymentId).stream().map(this::toDomain).toList();
  }
  ...
  public List<IdempotencyRecord> findByCreatedAtBefore(Instant beforeTime) {
    ...
    return jpaRepository.findByCreatedAtBefore(beforeTime).stream().map(this::toDomain).toList();
  }
