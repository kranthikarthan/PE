 ...
 *
 * <p>Provides repair and management APIs for payments in the payments engine. Enables operations
 * teams to retry, cancel, and manage failed payments.
 *
 * <p>Base URL: /api/repair/v1/payments
 */
@Slf4j
@RestController
...
public class PaymentRepairController {
   ...
   *
   * <p>POST /api/repair/v1/payments/{paymentId}/retry
   */
  ...
  public ResponseEntity<PaymentInitiationResponse> retryPayment(
      @Parameter(description = "Payment ID", required = true) @PathVariable("paymentId")
          String paymentId,
      @Parameter(description = "Retry request", required = true) @RequestBody @Valid
          RetryRequest retryRequest,
      @Parameter(description = "User ID", required = true) @RequestHeader("X-User-ID")
          String userId,
    ...
    log.info(
        "Retrying payment: {} by user: {}, tenant: {}, correlation: {}",
        paymentId,
        userId,
        tenantId,
        correlationId);
    ...
    try {
      PaymentInitiationResponse response =
          paymentRepairService.retryPayment(
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(PaymentInitiationResponse.builder().errorMessage("Payment retry failed").build());
    }
  }
   ...
   * Cancel a payment
   *
   * <p>POST /api/repair/v1/payments/{paymentId}/cancel
   */
  ...
  public ResponseEntity<PaymentInitiationResponse> cancelPayment(
      @Parameter(description = "Payment ID", required = true) @PathVariable("paymentId")
          String paymentId,
      @Parameter(description = "Cancel request", required = true) @RequestBody @Valid
    ...
    log.info(
        "Cancelling payment: {} by user: {}, tenant: {}, correlation: {}",
        paymentId,
        userId,
        tenantId,
        correlationId);
    ...
    try {
      PaymentInitiationResponse response =
          paymentRepairService.cancelPayment(
   ...
   *
   * <p>GET /api/repair/v1/payments/{paymentId}/repair-history
   */
  ...
  public ResponseEntity<?> getPaymentRepairHistory(
    ...
    log.info(
        "Retrieving repair history for payment: {}, tenant: {}, correlation: {}",
        paymentId,
        tenantId,
        correlationId);

    try {
      List<RepairHistoryItem> repairHistory =
          paymentRepairService.getPaymentRepairHistory(
              paymentId, correlationId, tenantId, businessUnitId);

      RepairHistoryResponse response =
          RepairHistoryResponse.builder()
              .paymentId(paymentId)
   ...
   *
   * <p>GET /api/repair/v1/payments/failed
   */
  ...
  public ResponseEntity<FailedPaymentsResponse> getFailedPayments(
      @Parameter(description = "Page number", required = false) @RequestParam(defaultValue = "0")
          int page,
      @Parameter(description = "Page size", required = false) @RequestParam(defaultValue = "20")
          int size,
      @Parameter(description = "Correlation ID for tracing", required = true)
          @RequestHeader("X-Correlation-ID")
          String correlationId,
    ...
    log.info(
        "Retrieving failed payments for tenant: {}, business unit: {}, correlation: {}",
        tenantId,
        businessUnitId,
        correlationId);

    try {
      List<PaymentInitiationResponse> failedPayments =
          paymentRepairService.getFailedPayments(
              page, size, correlationId, tenantId, businessUnitId);

      FailedPaymentsResponse response =
          FailedPaymentsResponse.builder()
   ...
   *
   * <p>POST /api/repair/v1/payments/bulk-retry
   */
  ...
  public ResponseEntity<BulkRetryResponse> bulkRetryPayments(
      @Parameter(description = "Bulk retry request", required = true) @RequestBody @Valid
    ...
    log.info(
        "Bulk retrying {} payments by user: {}, tenant: {}, correlation: {}",
        bulkRetryRequest.getPaymentIds().size(),
        userId,
        tenantId,
        correlationId);

    try {
      BulkRetryResponse response =
          paymentRepairService.bulkRetryPayments(
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(BulkRetryResponse.builder().errorMessage("Bulk retry failed").build());
    }
  }
