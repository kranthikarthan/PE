...
import com.payments.domain.payment.PaymentStatus as DomainPaymentStatus;
import com.payments.domain.payment.PaymentType as DomainPaymentType;
import com.payments.domain.payment.Priority as DomainPriority;
 ...
 * 
 * Tests mapping between domain objects and entities
 */
class PaymentMapperTest {
    ...
    void toEntity_ShouldMapDomainToEntity_WhenValidDomain() {
        ...
        assertThat(entity.getDestinationAccount()).isEqualTo(domainPayment.getDestinationAccount().getValue());
        ...
        assertThat(entity.getBusinessUnitId()).isEqualTo(domainPayment.getTenantContext().getBusinessUnitId());
    ...
    void toDomain_ShouldMapEntityToDomain_WhenValidEntity() {
        ...
        assertThat(domainPayment.getDestinationAccount().getValue()).isEqualTo(entity.getDestinationAccount());
        ...
        assertThat(domainPayment.getPaymentType()).isEqualTo(DomainPaymentType.valueOf(entity.getPaymentType()));
        assertThat(domainPayment.getPriority()).isEqualTo(DomainPriority.valueOf(entity.getPriority()));
        assertThat(domainPayment.getTenantContext().getTenantId()).isEqualTo(entity.getTenantId());
        assertThat(domainPayment.getTenantContext().getBusinessUnitId()).isEqualTo(entity.getBusinessUnitId());
        assertThat(domainPayment.getStatus()).isEqualTo(DomainPaymentStatus.valueOf(entity.getStatus()));
    ...
    void toResponse_ShouldMapDomainToResponse_WhenValidDomain() {
        ...
        assertThat(response.getStatus()).isEqualTo(PaymentStatus.valueOf(domainPayment.getStatus().name()));
    ...
    void toStatusHistoryEntities_ShouldMapStatusChanges_WhenValidDomain() {
        // Given
        Payment domainPayment = createDomainPayment();
        domainPayment.addStatusChange(DomainPaymentStatus.VALIDATED, "Validation successful", Instant.now());
        domainPayment.addStatusChange(DomainPaymentStatus.COMPLETED, "Payment completed", Instant.now());

        // When
        List<PaymentStatusHistoryEntity> entities = paymentMapper.toStatusHistoryEntities(domainPayment);
    ...
    void mapPaymentType_ShouldMapCorrectly_WhenValidTypes() {
        ...
        assertThat(paymentMapper.mapPaymentType(DomainPaymentType.EFT)).isEqualTo(PaymentType.EFT);
        assertThat(paymentMapper.mapPaymentType(DomainPaymentType.RTGS)).isEqualTo(PaymentType.RTGS);
        assertThat(paymentMapper.mapPaymentType(DomainPaymentType.CARD)).isEqualTo(PaymentType.CARD);
    }

    @Test
    void mapPriority_ShouldMapCorrectly_WhenValidPriorities() {
        // Test all priority mappings
        assertThat(paymentMapper.mapPriority(DomainPriority.LOW)).isEqualTo(Priority.LOW);
        assertThat(paymentMapper.mapPriority(DomainPriority.NORMAL)).isEqualTo(Priority.NORMAL);
        assertThat(paymentMapper.mapPriority(DomainPriority.HIGH)).isEqualTo(Priority.HIGH);
        assertThat(paymentMapper.mapPriority(DomainPriority.URGENT)).isEqualTo(Priority.URGENT);
    }

    @Test
    void mapStatus_ShouldMapCorrectly_WhenValidStatuses() {
        // Test all status mappings
        assertThat(paymentMapper.mapStatus(DomainPaymentStatus.INITIATED)).isEqualTo(PaymentStatus.INITIATED);
        assertThat(paymentMapper.mapStatus(DomainPaymentStatus.VALIDATED)).isEqualTo(PaymentStatus.VALIDATED);
        assertThat(paymentMapper.mapStatus(DomainPaymentStatus.COMPLETED)).isEqualTo(PaymentStatus.COMPLETED);
        assertThat(paymentMapper.mapStatus(DomainPaymentStatus.FAILED)).isEqualTo(PaymentStatus.FAILED);
    }
