 ...
 *
 * <p>Repository for routing rule operations: - Find rules by tenant and business unit - Find active
 * rules - Find rules by type and status - Custom queries for rule evaluation
 */
@Repository
public interface RoutingRuleRepository extends JpaRepository<RoutingRule, Long> {
   ...
   */
  @Query(
      "SELECT r FROM RoutingRule r WHERE r.tenantId = :tenantId "
   ...
   */
  @Query(
      "SELECT r FROM RoutingRule r WHERE r.effectiveTo IS NOT NULL AND r.effectiveTo <= :beforeDate")
   ...
   */
  @Query(
      "SELECT r FROM RoutingRule r WHERE r.effectiveFrom IS NOT NULL AND r.effectiveFrom > :afterDate")
   ...
   */
  @Query(
      "SELECT COUNT(r) FROM RoutingRule r WHERE r.tenantId = :tenantId "
          + "AND r.isActive = true "
          + "AND r.ruleStatus = 'ACTIVE' "
          + "AND (r.effectiveFrom IS NULL OR r.effectiveFrom <= :now) "
          + "AND (r.effectiveTo IS NULL OR r.effectiveTo > :now)")
   ...
   */
  @Query(
      "SELECT DISTINCT r FROM RoutingRule r "
