 ...
 *
 * <p>Repository for routing condition operations: - Find conditions by rule - Find conditions by
 * field name - Custom queries for condition evaluation
 */
@Repository
public interface RoutingConditionRepository extends JpaRepository<RoutingCondition, Long> {
   ...
   */
  @Query(
      "SELECT c FROM RoutingCondition c WHERE c.routingRule.id = :ruleId ORDER BY c.conditionOrder ASC")
