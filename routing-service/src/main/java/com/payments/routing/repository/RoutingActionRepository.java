 ...
 *
 * <p>Repository for routing action operations: - Find actions by rule - Find actions by type -
...
public interface RoutingActionRepository extends JpaRepository<RoutingAction, Long> {
   ...
   */
  @Query(
      "SELECT a FROM RoutingAction a WHERE a.routingRule.id = :ruleId ORDER BY a.actionOrder ASC")
   ...
   */
  @Query(
      "SELECT a FROM RoutingAction a WHERE a.routingRule.id = :ruleId AND a.isPrimary = true ORDER BY a.actionOrder ASC")
   ...
   */
  @Query(
      "SELECT a FROM RoutingAction a WHERE a.routingRule.id = :ruleId AND a.actionType = :actionType ORDER BY a.actionOrder ASC")
  List<RoutingAction> findByRuleIdAndActionType(
      @Param("ruleId") Long ruleId, @Param("actionType") RoutingActionType actionType);
   ...
   */
  @Query(
      "SELECT a FROM RoutingAction a WHERE a.routingRule.id = :ruleId ORDER BY a.routingPriority ASC, a.actionOrder ASC")
