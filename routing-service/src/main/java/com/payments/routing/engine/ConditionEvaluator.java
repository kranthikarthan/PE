 ...
 * Condition Evaluator
 * 
 * Evaluates routing conditions against payment attributes:
 * - Numeric comparisons (amount, priority)
 * - String comparisons (currency, payment type)
 * - Date/time comparisons
 * - Regex pattern matching
 * - List membership (IN, NOT_IN)
 * 
 * Performance: DSA-optimized data structures, efficient comparisons
 * Resilience: Graceful error handling, validation
 */
@Slf4j
@Component
public class ConditionEvaluator {
    ...
    public boolean evaluate(RoutingCondition condition, RoutingRequest request) {
        ...
        try {
            ...
            log.error("Error evaluating condition {}: {} with field value {}: {}",
                    condition.getFieldName(), condition.getOperator(), fieldValue, e.getMessage());
            return false;
        }
    }
    ...
    private Object getFieldValue(String fieldName, RoutingRequest request) {
        switch (fieldName) {
            case "paymentId": return request.getPaymentId();
            case "amount": return request.getAmount();
            case "currency": return request.getCurrency();
            case "paymentType": return request.getPaymentType();
            case "tenantId": return request.getTenantId();
            case "businessUnitId": return request.getBusinessUnitId();
            case "sourceAccount": return request.getSourceAccount();
            case "destinationAccount": return request.getDestinationAccount();
            case "priority": return request.getPriority();
            case "createdAt": return request.getCreatedAt();
            default:
    ...
    private int compareNumeric(Object fieldValue, String expectedValue) {
        ...
        try {
            ...
            log.error("Cannot compare non-numeric values as numbers: fieldValue='{}', expectedValue='{}'", 
    ...
    private boolean compareIn(Object fieldValue, String expectedValue) {
        List<String> expectedValues = Arrays.stream(expectedValue.split(","))
                .map(String::trim)
                .collect(Collectors.toList());
        return expectedValues.stream()
                .anyMatch(val -> val.equalsIgnoreCase(fieldValue.toString()));
    }
