 ...
 * Action Executor
 *
 * <p>Executes routing actions based on matched rules: - Route to clearing system - Set priority -
 * Add metadata - Reject payment - Hold payment - Send notifications
 *
 * <p>Performance: Efficient action processing Resilience: Graceful error handling
 */
@Slf4j
@Component
public class ActionExecutor {
  ...
  public RoutingDecision execute(RoutingRule rule, RoutingRequest request) {
    RoutingDecision.RoutingDecisionBuilder decisionBuilder =
        RoutingDecision.builder()
    ...
    Map<RoutingActionType, List<RoutingAction>> actionsByType =
        rule.getActions().stream().collect(Collectors.groupingBy(RoutingAction::getActionType));
    ...
    actionsByType.getOrDefault(RoutingActionType.ROUTE_TO_CLEARING_SYSTEM, List.of()).stream()
        ...
        .ifPresent(
            action -> {
              ...
              log.debug(
                  "Action: Route to clearing system '{}' for rule '{}'",
                  action.getClearingSystem(),
                  rule.getRuleName());
            });
    ...
    if (decisionBuilder.build().getClearingSystem() == null) {
      ...
      log.warn(
          "No primary ROUTE_TO_CLEARING_SYSTEM action found for rule '{}'. Using fallback: {}",
          rule.getRuleName(),
    ...
    actionsByType.getOrDefault(RoutingActionType.SET_PRIORITY, List.of()).stream()
        ...
        .ifPresent(
            action -> {
              ...
              log.debug(
                  "Action: Set priority to '{}' for rule '{}'",
    ...
    actionsByType
        .getOrDefault(RoutingActionType.ADD_METADATA, List.of())
        .forEach(
            action -> {
              if (action.getActionParameters() != null
                  && action.getActionParameters().contains("=")) {
    ...
    if (actionsByType.containsKey(RoutingActionType.REJECT_PAYMENT)) {
      ...
      actionsByType.get(RoutingActionType.REJECT_PAYMENT).stream()
          .findFirst()
          .ifPresent(
              action ->
                  decisionBuilder.decisionReason(
                      "Payment rejected by rule: "
                          + rule.getRuleName()
                          + ". "
                          + action.getActionParameters()));
    ...
    if (actionsByType.containsKey(RoutingActionType.HOLD_PAYMENT)) {
      ...
      actionsByType.get(RoutingActionType.HOLD_PAYMENT).stream()
          .findFirst()
          .ifPresent(
              action ->
                  decisionBuilder.decisionReason(
                      "Payment held by rule: "
                          + rule.getRuleName()
                          + ". "
                          + action.getActionParameters()));
    ...
    actionsByType
        .getOrDefault(RoutingActionType.NOTIFY, List.of())
        .forEach(
            action -> {
