 ...
 * Routing Service
 *
 * <p>Core business logic for payment routing: - Route payments to appropriate clearing systems -
 * Cache routing decisions for performance - Provide routing statistics - Handle fallback scenarios
 *
 * <p>Performance: Redis caching, DSA-optimized data structures Resilience: Istio for internal calls
 * (not Resilience4j)
 */
@Slf4j
@Service
...
public class RoutingService {
  ...
  public RoutingDecision getRoutingDecision(RoutingRequest request) {
    ...
    statistics
        .computeIfAbsent(decision.getClearingSystem(), k -> new AtomicLong(0))
        .incrementAndGet();
   ...
   */
  @Transactional(readOnly = true)
  public RoutingDecision getRoutingDecisionWithFallback(
      RoutingRequest request, String fallbackClearingSystem) {
    ...
    try {
      ...
      if (decision.getClearingSystem() == null || decision.getClearingSystem().isEmpty()) {
        log.warn(
            "No clearing system found for payment: {}, using fallback: {}",
            request.getPaymentId(),
