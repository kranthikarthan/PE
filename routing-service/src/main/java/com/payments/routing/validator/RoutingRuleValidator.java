 ...
 * 
 * Validates routing rules for correctness:
...
public class RoutingRuleValidator {
     ...
     */
    private void validateRuleProperties(RoutingRule rule, List<String> errors, List<String> warnings) {
        // Validate rule name
     ...
     * @param warnings Warning list
     */
    private void validateConditions(List<RoutingCondition> conditions, List<String> errors, List<String> warnings) {
     ...
     * @param warnings Warning list
     */
    private void validateCondition(RoutingCondition condition, int index, List<String> errors, List<String> warnings) {
        // Validate field name
     ...
     * @param warnings Warning list
     */
    private void validateFieldValueType(RoutingCondition condition, int index, List<String> errors, List<String> warnings) {
     ...
     * @param warnings Warning list
     */
    private void validateConditionOrder(List<RoutingCondition> conditions, List<String> errors, List<String> warnings) {
        for (int i = 0; i < conditions.size(); i++) {
            RoutingCondition condition = conditions.get(i);
            if (condition.getConditionOrder() == null) {
                ...
                warnings.add("Condition " + i + ": Condition order should be " + (i + 1) + " but is " + condition.getConditionOrder());
            }
        }
    }
     ...
     * @param warnings Warning list
     */
    private void validateActions(List<RoutingAction> actions, List<String> errors, List<String> warnings) {
        if (actions.isEmpty()) {
     ...
     * @param warnings Warning list
     */
    private void validateAction(RoutingAction action, int index, List<String> errors, List<String> warnings) {
     ...
     * @param warnings Warning list
     */
    private void validateActionOrder(List<RoutingAction> actions, List<String> errors, List<String> warnings) {
        for (int i = 0; i < actions.size(); i++) {
            RoutingAction action = actions.get(i);
            if (action.getActionOrder() == null) {
                errors.add("Action " + i + ": Action order is required");
            } else if (action.getActionOrder() != i + 1) {
                warnings.add("Action " + i + ": Action order should be " + (i + 1) + " but is " + action.getActionOrder());
            }
        }
    }
     ...
     * @param warnings Warning list
     */
    private void validatePrimaryAction(List<RoutingAction> actions, List<String> errors, List<String> warnings) {
        long primaryCount = actions.stream()
                .mapToLong(action -> action.isPrimary() ? 1 : 0)
                .sum();
