 ...
 * Routing Controller
 *
 * <p>REST API for payment routing decisions: - Route payment to appropriate clearing system - Get
 * routing statistics - Manage routing cache
 *
 * <p>Security: JWT validation, tenant isolation Performance: Redis caching, DSA-optimized data
 * structures Resilience: Istio for internal calls (not Resilience4j)
 */
@Slf4j
@RestController
...
@Tag(
    ...
    description = "Payment routing decisions and clearing system selection")
public class RoutingController {
  ...
  @Operation(
      ...
      description =
          "Determine the appropriate clearing system for a payment based on routing rules")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
  ...
  public ResponseEntity<RoutingDecision> getRoutingDecision(
    ...
    log.info(
        "Getting routing decision for payment: {} with tenant: {}",
    ...
    try {
      ...
      log.info(
          "Routing decision made for payment: {} - clearing system: {}, priority: {}",
          request.getPaymentId(),
  ...
  public ResponseEntity<RoutingDecision> getRoutingDecisionWithFallback(
      @Valid @RequestBody RoutingRequest request,
      @RequestParam("fallbackClearingSystem") @Parameter(description = "Fallback clearing system")
          String fallbackClearingSystem,
      @RequestHeader("X-Tenant-ID") @Parameter(description = "Tenant ID") String tenantId,
      @RequestHeader(value = "X-Correlation-ID", required = false)
          @Parameter(description = "Correlation ID")
          String correlationId) {
    ...
    log.info(
        "Getting routing decision with fallback for payment: {} with tenant: {}, fallback: {}",
        request.getPaymentId(),
        tenantId,
        fallbackClearingSystem);
    ...
    try {
      ...
      RoutingDecision decision =
          routingService.getRoutingDecisionWithFallback(request, fallbackClearingSystem);
      ...
      log.error(
          "Error getting routing decision with fallback for payment: {}",
          request.getPaymentId(),
          e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
    }
  ...
  public ResponseEntity<Map<String, String>> clearRoutingCache(
    ...
    try {
      ...
      return ResponseEntity.ok(
          Map.of("message", "Cache cleared successfully", "paymentId", paymentId));
   ...
   */
  @DeleteMapping("/cache")
  @Operation(summary = "Clear all routing cache", description = "Clear all routing cache entries")
  @ApiResponses({
  ...
  public static class RoutingStatistics {
    ...
    public Double getAverageDecisionTime() {
      return averageDecisionTime;
    }

    public void setAverageDecisionTime(Double averageDecisionTime) {
