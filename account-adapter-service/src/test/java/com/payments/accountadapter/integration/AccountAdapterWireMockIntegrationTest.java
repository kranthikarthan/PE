 ...
 *
 * <p>Integration tests using WireMock for external service mocking: - Account balance retrieval -
 * Account validation - Account status checking - Error scenarios - Circuit breaker testing
 */
...
class AccountAdapterWireMockIntegrationTest {
  ...
  @Autowired private AccountCacheService accountCacheService;

  @Autowired private OAuth2TokenService oAuth2TokenService;
  ...
  void testGetAccountBalance_Success() {
    ...
    wireMockServer.verify(
        getRequestedFor(urlEqualTo("/api/v1/accounts/balance"))
  ...
  void testValidateAccount_Success() {
    ...
    // When
    ResponseEntity<AccountValidationResponse> response =
        restTemplate.exchange(
    ...
    // Verify WireMock was called
    wireMockServer.verify(
        postRequestedFor(urlEqualTo("/api/v1/accounts/validate"))
            .withHeader("Authorization", matching("Bearer.*"))
            .withHeader("X-Correlation-ID", equalTo(correlationId))
            .withHeader("X-Tenant-ID", equalTo(tenantId)));
  }
  ...
  void testGetAccountStatus_Success() {
    ...
    // When
    ResponseEntity<AccountStatusResponse> response =
        restTemplate.exchange(
    ...
    // Verify WireMock was called
    wireMockServer.verify(
        getRequestedFor(urlEqualTo("/api/v1/accounts/status"))
            .withHeader("Authorization", matching("Bearer.*"))
            .withHeader("X-Correlation-ID", equalTo(correlationId))
            .withHeader("X-Tenant-ID", equalTo(tenantId)));
  }
