 ...
 * 
 * Service for caching account data in Redis:
 * - Account balance caching
 * - Account validation caching
...
public class AccountCacheService {
     ...
     */
    public void cacheAccountBalance(String accountNumber, String tenantId, AccountBalanceResponse response) {
        try {
     ...
     */
    public void cacheAccountValidation(String accountNumber, String tenantId, AccountValidationResponse response) {
        try {
            String cacheKey = VALIDATION_CACHE_PREFIX + tenantId + ":" + accountNumber;
            redisTemplate.opsForValue().set(cacheKey, response, Duration.ofMillis(cacheTtl));
            log.debug("Cached account validation for account: {} with TTL: {}ms", accountNumber, cacheTtl);
        } catch (Exception e) {
     ...
     */
    public AccountValidationResponse getCachedAccountValidation(String accountNumber, String tenantId) {
        try {
            String cacheKey = VALIDATION_CACHE_PREFIX + tenantId + ":" + accountNumber;
            Object cached = redisTemplate.opsForValue().get(cacheKey);
     ...
     */
    public void cacheAccountStatus(String accountNumber, String tenantId, AccountStatusResponse response) {
        try {
