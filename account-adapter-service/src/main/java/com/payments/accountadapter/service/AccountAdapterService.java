 ...
 *
 * <p>Service for account system integration with resilience patterns: - Circuit breaker protection
 * - Retry logic - Timeout handling - Fallback mechanisms
 */
@Slf4j
@Service
...
public class AccountAdapterService {
  ...
  public CompletableFuture<AccountBalanceResponse> getAccountBalance(
    ...
    try {
      // Check cache first
      AccountBalanceResponse cachedResponse =
          accountCacheService.getCachedAccountBalance(accountNumber, tenantId);
      ...
      AccountBalanceResponse response =
          accountServiceClient.getAccountBalance(
              request, "Bearer " + accessToken, correlationId, tenantId);

      // Cache the response
      accountCacheService.cacheAccountBalance(accountNumber, tenantId, response);

      log.info(
          "Successfully retrieved account balance for account: {} with correlation: {}",
          accountNumber,
          correlationId);
  ...
  public CompletableFuture<AccountValidationResponse> validateAccount(
    ...
    try {
      // Check cache first
      AccountValidationResponse cachedResponse =
          accountCacheService.getCachedAccountValidation(accountNumber, tenantId);
      if (cachedResponse != null) {
      ...
      log.info(
          "Successfully validated account: {} with correlation: {}", accountNumber, correlationId);

      return CompletableFuture.completedFuture(response);

    } catch (Exception e) {
      log.error(
          "Failed to validate account: {} with correlation: {}", accountNumber, correlationId, e);
      throw new AccountServiceException(
  ...
  public CompletableFuture<AccountStatusResponse> getAccountStatus(
    ...
    try {
      // Check cache first
      AccountStatusResponse cachedResponse =
          accountCacheService.getCachedAccountStatus(accountNumber, tenantId);
      if (cachedResponse != null) {
      ...
      AccountStatusResponse response =
          accountServiceClient.getAccountStatus(
              request, "Bearer " + accessToken, correlationId, tenantId);
      ...
      log.info(
          "Successfully retrieved account status for account: {} with correlation: {}",
          accountNumber,
          correlationId);
  ...
  public CompletableFuture<AccountBalanceResponse> getAccountBalanceFallback(
    ...
    log.warn(
        "Using fallback for account balance: {} with correlation: {}",
        accountNumber,
        correlationId);
    ...
    AccountBalanceResponse response =
        accountServiceFallback.getAccountBalance(
  ...
  public CompletableFuture<AccountValidationResponse> validateAccountFallback(
    ...
    log.warn(
        "Using fallback for account validation: {} with correlation: {}",
        accountNumber,
        correlationId);
    ...
    AccountValidationResponse response =
        accountServiceFallback.validateAccount(request, "Bearer fallback", correlationId, tenantId);
  ...
  public CompletableFuture<AccountStatusResponse> getAccountStatusFallback(
    ...
    log.warn(
        "Using fallback for account status: {} with correlation: {}", accountNumber, correlationId);
    ...
    AccountStatusResponse response =
        accountServiceFallback.getAccountStatus(
            request, "Bearer fallback", correlationId, tenantId);
