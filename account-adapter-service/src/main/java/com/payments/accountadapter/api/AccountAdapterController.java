 ...
 * 
 * REST controller for account adapter operations:
...
public class AccountAdapterController {
    ...
    @Operation(summary = "Get account balance", description = "Retrieve account balance with resilience patterns")
    @ApiResponses(value = {
    ...
    public CompletableFuture<ResponseEntity<AccountBalanceResponse>> getAccountBalance(
        ...
        log.info("Getting account balance for account: {} with correlation: {}", 
                accountNumber, actualCorrelationId);
        
        return accountAdapterService.getAccountBalance(accountNumber, tenantId, businessUnitId, actualCorrelationId)
    ...
    @Operation(summary = "Validate account", description = "Validate account with resilience patterns")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Account validation completed successfully"),
            @ApiResponse(responseCode = "400", description = "Invalid request parameters"),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public CompletableFuture<ResponseEntity<AccountValidationResponse>> validateAccount(
        ...
        return accountAdapterService.validateAccount(accountNumber, tenantId, businessUnitId, actualCorrelationId)
                .thenApply(ResponseEntity::ok)
                .exceptionally(throwable -> {
    ...
    @Operation(summary = "Get account status", description = "Retrieve account status with resilience patterns")
    @ApiResponses(value = {
    ...
    public CompletableFuture<ResponseEntity<AccountStatusResponse>> getAccountStatus(
        ...
        log.info("Getting account status for account: {} with correlation: {}", 
                accountNumber, actualCorrelationId);
        
        return accountAdapterService.getAccountStatus(accountNumber, tenantId, businessUnitId, actualCorrelationId)
                .thenApply(ResponseEntity::ok)
                .exceptionally(throwable -> {
