...
class ValidationResultServiceTest {
  ...
  @Mock private ValidationResultRepository repository;

  @Mock private ValidationResultMapper mapper;

  @InjectMocks private ValidationResultService service;
  ...
  void findByTenantId_WithValidId_ShouldReturnPage() {
    ...
    when(repository.findByTenantIdOrderByValidatedAtDesc("tenant-1", pageable))
  ...
  void findByStatus_WithValidStatus_ShouldReturnPage() {
    ...
    Page<ValidationResultEntity> entityPage = new PageImpl<>(List.of(validEntity));
    Pageable pageable = PageRequest.of(0, 10);
    when(repository.findByStatusOrderByValidatedAtDesc(any(), any(Pageable.class)))
        .thenReturn(entityPage);
    when(mapper.toDomain(validEntity)).thenReturn(validResult);
  ...
  void findByRiskLevel_WithValidLevel_ShouldReturnPage() {
    ...
    Page<ValidationResultEntity> entityPage = new PageImpl<>(List.of(validEntity));
    Pageable pageable = PageRequest.of(0, 10);
    when(repository.findByRiskLevelOrderByValidatedAtDesc(any(), any(Pageable.class)))
        .thenReturn(entityPage);
    when(mapper.toDomain(validEntity)).thenReturn(validResult);
  ...
  void getValidationStatistics_WithValidTenant_ShouldReturnStatistics() {
    ...
    // When
    ValidationResultService.ValidationStatistics result =
        service.getValidationStatistics("tenant-1");
