...
class RiskAssessmentRuleEngineTest {
    ...
    void executeRules_WithValidPayment_ShouldPass() {
        ...
        // When
        RuleExecutionFacade.RuleExecutionResult result = riskAssessmentRuleEngine.executeRules(context, event);

        // Then
        assertThat(result).isNotNull();
        assertThat(result.getRuleType()).isEqualTo(RuleType.RISK);
        assertThat(result.isSuccess()).isTrue();
        assertThat(result.getAppliedRules()).contains("RISK_RULE_001", "RISK_RULE_002", "RISK_RULE_003", "RISK_RULE_004", "RISK_RULE_005");
    ...
    void executeRules_WithHighCreditRisk_ShouldDetectRisk() {
        ...
        assertThat(result.getFailedRules().get(0).getFailureReason()).contains("High credit risk transaction");
    ...
    void executeRules_WithForeignCurrency_ShouldDetectRisk() {
        ...
        assertThat(result.getFailedRules().get(0).getRuleType()).isEqualTo(RuleType.RISK);
        assertThat(result.getFailedRules().get(0).getFailureReason()).contains("Foreign currency transaction");
    ...
    void executeRules_WithHighOperationalRisk_ShouldDetectRisk() {
        ...
        assertThat(result.getFailedRules().get(0).getRuleName()).isEqualTo("Operational Risk Evaluation");
        assertThat(result.getFailedRules().get(0).getRuleType()).isEqualTo(RuleType.RISK);
        assertThat(result.getFailedRules().get(0).getFailureReason()).contains("High-value transaction");
    ...
    void executeRules_WithHighCounterpartyRisk_ShouldDetectRisk() {
        ...
        assertThat(result.getFailedRules().get(0).getRuleName()).isEqualTo("Counterparty Risk Analysis");
        assertThat(result.getFailedRules().get(0).getRuleType()).isEqualTo(RuleType.RISK);
        assertThat(result.getFailedRules().get(0).getFailureReason()).contains("High-risk counterparty");
    ...
    private PaymentInitiatedEvent createHighCounterpartyRiskPaymentEvent() {
        PaymentInitiatedEvent event = createValidPaymentEvent();
        event.setDestinationAccount("RISK1234567"); // Contains "RISK" which triggers high-risk counterparty
        return event;
    }
