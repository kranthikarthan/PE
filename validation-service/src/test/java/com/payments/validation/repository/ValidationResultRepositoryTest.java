...
class ValidationResultRepositoryTest {
  ...
  @Autowired private ValidationResultRepository repository;
  ...
  void setUp() {
    // Create test data
    validResult =
        ValidationResultEntity.builder()
  ...
  void findByTenantId_WithValidId_ShouldReturnResults() {
    // When
    Page<ValidationResultEntity> results =
        repository.findByTenantId("tenant-1", PageRequest.of(0, 10));

    // Then
    assertThat(results.getContent()).hasSize(2);
    assertThat(results.getContent())
        .extracting(ValidationResultEntity::getTenantId)
  ...
  void findByTenantIdAndBusinessUnitId_WithValidIds_ShouldReturnResults() {
    // When
    Page<ValidationResultEntity> results =
        repository.findByTenantIdAndBusinessUnitId(
    ...
    assertThat(results.getContent())
        ...
        .containsOnly("tenant-1");
    assertThat(results.getContent())
        .extracting(ValidationResultEntity::getBusinessUnitId)
  ...
  void findByStatus_WithValidStatus_ShouldReturnResults() {
    // When
    Page<ValidationResultEntity> results =
        repository.findByStatus(ValidationStatus.PASSED, PageRequest.of(0, 10));

    // Then
    assertThat(results.getContent()).hasSize(2);
    assertThat(results.getContent())
        .extracting(ValidationResultEntity::getStatus)
  ...
  void findByRiskLevel_WithValidLevel_ShouldReturnResults() {
    // When
    Page<ValidationResultEntity> results =
        repository.findByRiskLevel(RiskLevel.CRITICAL, PageRequest.of(0, 10));
  ...
  void findByFraudScoreGreaterThanEqual_WithThreshold_ShouldReturnResults() {
    // When
    Page<ValidationResultEntity> results =
        repository.findByFraudScoreGreaterThanEqual(50, PageRequest.of(0, 10));
  ...
  void findByRiskScoreGreaterThanEqual_WithThreshold_ShouldReturnResults() {
    // When
    Page<ValidationResultEntity> results =
        repository.findByRiskScoreGreaterThanEqual(50, PageRequest.of(0, 10));
