 ...
 *
 * <p>Executes validation rules against payment events: - Business rule validation - Compliance rule
 * validation - Fraud detection rules - Risk assessment rules
 */
@Slf4j
@Service
...
public class ValidationRuleEngine {
  ...
  public ValidationResult executeValidation(
    ...
    log.info(
        "Executing validation rules for payment: {} in context: {}",
  ...
  private int calculateFraudScore(List<FailedRule> failedRules) {
    return (int)
            failedRules.stream()
                .filter(rule -> RuleType.FRAUD.toString().equals(rule.getRuleType()))
                .count()
        * 25; // Each fraud rule adds 25 points
  }
  ...
  private int calculateRiskScore(List<FailedRule> failedRules) {
    return (int)
            failedRules.stream()
                .filter(rule -> RuleType.RISK.toString().equals(rule.getRuleType()))
                .count()
        * 20; // Each risk rule adds 20 points
  }
  ...
  private String createValidationMetadata(ValidationContext context, PaymentInitiatedEvent event) {
    ...
    return String.format(
        "{\"validationId\":\"%s\",\"paymentId\":\"%s\",\"tenantId\":\"%s\"}",
        context.getValidationId(),
        event.getPaymentId().getValue(),
