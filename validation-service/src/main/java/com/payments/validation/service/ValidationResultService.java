 ...
 *
 * <p>Provides business logic for: - Saving validation results - Retrieving validation results -
 * Analytics and reporting - Data cleanup operations
 */
@Slf4j
@Service
...
public class ValidationResultService {
  ...
  public ValidationResult saveValidationResult(ValidationResult validationResult) {
    log.debug(
        "Saving validation result: {} for payment: {}",
        validationResult.getValidationId(),
        validationResult.getPaymentId().getValue());
    ...
    try {
      ...
      log.info(
          "Successfully saved validation result: {} for payment: {}",
          savedEntity.getValidationId(),
  ...
  public Optional<ValidationResult> findByValidationId(String validationId) {
    ...
    return repository.findByValidationId(validationId).map(mapper::toDomain);
  }
  ...
  public List<ValidationResult> findByPaymentId(String paymentId) {
    ...
    return repository.findByPaymentIdOrderByValidatedAtDesc(paymentId).stream()
        .map(mapper::toDomain)
        .toList();
  }
  ...
  public Page<ValidationResult> findByTenantId(String tenantId, Pageable pageable) {
    ...
    return repository
        .findByTenantIdOrderByValidatedAtDesc(tenantId, pageable)
        .map(mapper::toDomain);
  }
  ...
  public Page<ValidationResult> findByTenantIdAndBusinessUnitId(
    ...
    log.debug(
        "Finding validation results by tenant: {} and business unit: {}", tenantId, businessUnitId);

    return repository
        .findByTenantIdAndBusinessUnitIdOrderByValidatedAtDesc(tenantId, businessUnitId, pageable)
        .map(mapper::toDomain);
  }
  ...
  public List<ValidationResult> findByCorrelationId(String correlationId) {
    ...
    return repository.findByCorrelationIdOrderByValidatedAtDesc(correlationId).stream()
        .map(mapper::toDomain)
        .toList();
  }
  ...
  public Page<ValidationResult> findByStatus(ValidationStatus status, Pageable pageable) {
    ...
    return repository
        .findByStatusOrderByValidatedAtDesc(entityStatus, pageable)
        .map(mapper::toDomain);
  }
  ...
  public Page<ValidationResult> findByRiskLevel(RiskLevel riskLevel, Pageable pageable) {
    ...
    return repository
        .findByRiskLevelOrderByValidatedAtDesc(entityRiskLevel, pageable)
        .map(mapper::toDomain);
  }
  ...
  public Page<ValidationResult> findByValidatedAtBetween(
    ...
    return repository.findByValidatedAtBetween(startDate, endDate, pageable).map(mapper::toDomain);
  }
