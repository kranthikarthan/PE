 ...
 *
 * <p>Executes business validation rules: - Amount limits and thresholds - Account validation rules
 * - Business logic constraints - Operational rules
 */
@Slf4j
@Service
...
public class BusinessRuleEngine {
    ...
    if (amount > maxAmount) {
      failedRules.add(
          FailedRule.builder()
              ...
              .failureReason(
                  String.format(
                      "Payment amount %.2f exceeds maximum limit %.2f", amount, maxAmount))
  ...
  private void executeAccountValidationRule(
      PaymentInitiatedEvent event, List<String> appliedRules, List<FailedRule> failedRules) {
  ...
  private void executeBusinessHoursRule(
      PaymentInitiatedEvent event, List<String> appliedRules, List<FailedRule> failedRules) {
  ...
  private void executePaymentTypeValidationRule(
      PaymentInitiatedEvent event, List<String> appliedRules, List<FailedRule> failedRules) {
    ...
    // For now, just log that the rule was applied
    log.debug(
        "Payment type validation rule applied for payment: {}", event.getPaymentId().getValue());
  }
