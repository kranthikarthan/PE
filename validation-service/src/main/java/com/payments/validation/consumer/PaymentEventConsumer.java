 ...
 *
 * <p>Consumes payment events from Kafka topics: - PaymentInitiatedEvent: Process payment validation
 * - PaymentUpdatedEvent: Re-validate updated payments - AccountChangedEvent: Update validation
 * rules
 */
@Slf4j
@Component
  ...
  public void handlePaymentInitiated(
    ...
    try {
      ...
      String correlationId =
          extractAndValidateCorrelationId(headers, event.getEventId().toString());
      String tenantId = extractAndValidateTenantId(headers, event.getEventId().toString());
      String businessUnitId =
          extractAndValidateBusinessUnitId(headers, event.getEventId().toString());
      ...
      log.debug(
          "Processing payment validation for payment: {}, tenant: {}, correlation: {}",
      ...
      log.info(
          "Successfully processed PaymentInitiatedEvent: {} with correlation: {}",
          event.getEventId(),
          correlationId);

    } catch (Exception e) {
      log.error(
          "Failed to process PaymentInitiatedEvent: {} with correlation: {}",
          event.getEventId(),
          correlationService.getCurrentCorrelationId(),
          e);
