 ...
 *
 * <p>Produces validation result events to Kafka topics: - PaymentValidatedEvent: Successful
 * validation results - ValidationFailedEvent: Failed validation results - FraudDetectedEvent: Fraud
 * detection alerts - RiskAssessmentEvent: Risk assessment results
 */
@Slf4j
@Component
...
public class ValidationEventProducer {
  ...
  public void publishPaymentValidated(
      PaymentValidatedEvent event, String correlationId, String tenantId, String businessUnitId) {

    log.info(
        "Publishing PaymentValidatedEvent: {} for payment: {}",
        event.getEventId(),
  ...
  public void publishValidationFailed(
      ValidationFailedEvent event, String correlationId, String tenantId, String businessUnitId) {

    log.info(
        "Publishing ValidationFailedEvent: {} for payment: {}",
        event.getEventId(),
        event.getPaymentId().getValue());
  ...
  public void publishFraudDetected(
    ...
    log.warn("Publishing FraudDetectedEvent for payment: {} with score: {}", paymentId, fraudScore);
  ...
  public void publishRiskAssessment(
    ...
    log.info(
        "Publishing RiskAssessmentEvent for payment: {} with level: {} and score: {}",
        paymentId,
        riskLevel,
        riskScore);
  ...
  private void sendEvent(
      String topic, String key, Object event, Map<String, Object> headers, String eventType) {
    ...
    try {
      ...
      future.whenComplete(
          (result, ex) -> {
            if (ex == null) {
              log.debug(
                  "Successfully sent {} to topic: {}, partition: {}, offset: {}",
                  eventType,
                  topic,
