...
class TransactionProcessingIntegrationTest {
    ...
    void createTransaction_ShouldPersistTransactionAndEvents() {
        ...
        TransactionEntity savedEntity = transactionRepository.findByTenantContextAndId(tenantContext, transactionId);
    ...
    void completeTransactionLifecycle_ShouldWorkEndToEnd() {
        ...
        Transaction processingTransaction = transactionService.startProcessing(transactionId, tenantContext);
        assertEquals(TransactionStatus.PROCESSING, processingTransaction.getStatus());

        // When - Mark as cleared
        Transaction clearedTransaction = transactionService.markAsCleared(
            transactionId, tenantContext, "CHAPS", "CHAPS-REF-123"
        );
        assertEquals(TransactionStatus.CLEARED, clearedTransaction.getStatus());

        // When - Complete transaction
        Transaction completedTransaction = transactionService.completeTransaction(transactionId, tenantContext);
    ...
    void failTransaction_ShouldPersistFailureEvent() {
        ...
        Transaction failedTransaction = transactionService.failTransaction(transactionId, tenantContext, failureReason);
    ...
    void getTenantTransactions_ShouldReturnCorrectTransactions() {
        ...
        transactionService.createTransaction(
            transactionId1, tenantContext, paymentId, debitAccount, creditAccount, amount, TransactionType.PAYMENT
        );
        transactionService.createTransaction(
            transactionId2, tenantContext, paymentId, debitAccount, creditAccount, amount, TransactionType.PAYMENT
        );
    ...
    void getTenantTransactionsByStatus_ShouldFilterCorrectly() {
        ...
        List<Transaction> createdTransactions = transactionService.getTenantTransactionsByStatus(
            tenantContext, TransactionStatus.CREATED
        );
        List<Transaction> processingTransactions = transactionService.getTenantTransactionsByStatus(
    ...
    void eventPublishing_ShouldBeTriggered() {
        ...
        // verify(kafkaTemplate, atLeastOnce()).send(any(String.class), any(String.class), any(Object.class));
    }
}
