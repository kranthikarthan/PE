...
public interface TransactionRepository extends JpaRepository<TransactionEntity, TransactionId> {
    ...
    Optional<TransactionEntity> findByIdAndTenantContext(TransactionId id, TenantContext tenantContext);

    List<TransactionEntity> findByTenantContext(TenantContext tenantContext);

    List<TransactionEntity> findByPaymentIdAndTenantContext(PaymentId paymentId, TenantContext tenantContext);

    List<TransactionEntity> findByStatusAndTenantContext(TransactionStatus status, TenantContext tenantContext);

    @Query("SELECT t FROM TransactionEntity t WHERE t.tenantContext = :tenantContext " +
    ...
    @Query("SELECT COUNT(t) FROM TransactionEntity t WHERE t.tenantContext = :tenantContext " +
           "AND t.status = :status")
    ...
    @Query("SELECT t FROM TransactionEntity t WHERE t.clearingSystem = :clearingSystem " +
