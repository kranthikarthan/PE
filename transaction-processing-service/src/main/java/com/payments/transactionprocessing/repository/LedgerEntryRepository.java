...
public interface LedgerEntryRepository extends JpaRepository<LedgerEntryEntity, LedgerEntryId> {
  ...
  List<LedgerEntryEntity> findByTenantContextAndAccountNumber(
      TenantContext tenantContext, AccountNumber accountNumber);

  List<LedgerEntryEntity> findByTenantContextAndAccountNumberAndEntryType(
      TenantContext tenantContext, AccountNumber accountNumber, LedgerEntryType entryType);

  @Query(
      "SELECT le FROM LedgerEntryEntity le WHERE le.tenantContext = :tenantContext "
  ...
  @Query(
      "SELECT SUM(le.amount) FROM LedgerEntryEntity le WHERE le.tenantContext = :tenantContext "
          + "AND le.accountNumber = :accountNumber "
  ...
  @Query(
      "SELECT le FROM LedgerEntryEntity le WHERE le.transaction.id = :transactionId "
  ...
  @Query(
      "SELECT DISTINCT le.accountNumber FROM LedgerEntryEntity le WHERE le.tenantContext = :tenantContext")
  List<String> findDistinctAccountNumbersByTenantContext(
      @Param("tenantContext") TenantContext tenantContext);

  List<LedgerEntryEntity> findByTenantContext(TenantContext tenantContext);

  @Query(
      "SELECT COUNT(le) FROM LedgerEntryEntity le WHERE le.tenantContext = :tenantContext "
          + "AND le.accountNumber = :accountNumber")
