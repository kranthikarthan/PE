...
public class BalanceController {
    ...
    @Operation(summary = "Validate system balance", description = "Validates that all account balances are consistent")
    ...
    @Operation(summary = "Reconcile all balances", description = "Performs comprehensive balance reconciliation")
    ...
    @Operation(summary = "Reconcile balances for period", description = "Reconciles balances for a specific date range")
    public ResponseEntity<BalanceReconciliationService.ReconciliationResult> reconcileBalancesForPeriod(
            @Parameter(description = "Tenant ID") @PathVariable String tenantId,
            @Parameter(description = "Business Unit ID") @RequestParam String businessUnitId,
        ...
        TenantContext tenantContext = TenantContext.of(tenantId, "Default Tenant", businessUnitId, "Default Business Unit");
        BalanceReconciliationService.ReconciliationResult result = 
                balanceReconciliationService.reconcileBalancesForDateRange(tenantContext, startDate, endDate);
        
        return ResponseEntity.ok(result);
    }

    @PostMapping("/fix/{tenantId}")
    @Operation(summary = "Fix balance discrepancies", description = "Fixes balance discrepancies by updating cached balances")
    public ResponseEntity<Map<String, Object>> fixBalanceDiscrepancies(
        ...
        return ResponseEntity.ok(Map.of(
            "tenantId", tenantId,
            "status", "discrepancies_fixed",
            "timestamp", java.time.Instant.now()
        ));
    }

    @GetMapping("/integrity/{tenantId}")
    @Operation(summary = "Validate ledger integrity", description = "Validates the integrity of the ledger")
