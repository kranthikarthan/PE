...
@Slf4j
@Tag(name = "Transaction Management", description = "API for managing financial transactions and ledger entries")
public class TransactionController {
    ...
    @PostMapping
    @Operation(summary = "Create a new transaction", description = "Creates a new financial transaction with double-entry bookkeeping")
    public ResponseEntity<TransactionResponse> createTransaction(
            @Valid @RequestBody CreateTransactionRequest request) {
        
        log.info("Creating transaction for payment {} with amount {}", 
                request.getPaymentId(), request.getAmount());

        TransactionId transactionId = TransactionId.generate();
        TenantContext tenantContext = TenantContext.of(request.getTenantId(), "Default Tenant", request.getBusinessUnitId(), "Default Business Unit");
    ...
    public ResponseEntity<TransactionResponse> getTransaction(
        ...
        TenantContext tenantContext = TenantContext.of(tenantId, "Default Tenant", businessUnitId, "Default Business Unit");
        Optional<Transaction> transaction = transactionService.findById(
            TransactionId.of(transactionId), tenantContext);
    ...
    @Operation(summary = "Start processing transaction", description = "Marks a transaction as processing")
    public ResponseEntity<TransactionResponse> startProcessing(
        ...
        TenantContext tenantContext = TenantContext.of(tenantId, "Default Tenant", businessUnitId, "Default Business Unit");
        Transaction transaction = transactionService.startProcessing(
            TransactionId.of(transactionId), tenantContext);
    ...
    @Operation(summary = "Mark transaction as cleared", description = "Marks a transaction as cleared with clearing system details")
    public ResponseEntity<TransactionResponse> markCleared(
        ...
        log.info("Marking transaction {} as cleared with system {}", transactionId, request.getClearingSystem());

        TenantContext tenantContext = TenantContext.of(tenantId, "Default Tenant", businessUnitId, "Default Business Unit");
        Transaction transaction = transactionService.markCleared(
    ...
    @Operation(summary = "Fail transaction", description = "Marks a transaction as failed with reason")
    public ResponseEntity<TransactionResponse> failTransaction(
        ...
        TenantContext tenantContext = TenantContext.of(tenantId, "Default Tenant", businessUnitId, "Default Business Unit");
        Transaction transaction = transactionService.failTransaction(
    ...
    @GetMapping
    @Operation(summary = "Get transactions", description = "Retrieves transactions based on various criteria")
    ...
    @Operation(summary = "Get transaction ledger entries", description = "Retrieves all ledger entries for a transaction")
    public ResponseEntity<List<LedgerEntryResponse>> getTransactionLedgerEntries(
        ...
        List<LedgerEntry> entries = ledgerService.getTransactionEntries(TransactionId.of(transactionId));
    ...
    @Operation(summary = "Get transaction events", description = "Retrieves all events for a transaction")
    public ResponseEntity<List<TransactionEventResponse>> getTransactionEvents(
        ...
        List<TransactionEvent> events = eventService.getTransactionEvents(TransactionId.of(transactionId));
