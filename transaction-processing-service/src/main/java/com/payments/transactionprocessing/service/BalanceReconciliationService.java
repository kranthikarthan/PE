...
public class BalanceReconciliationService {
  ...
  public ReconciliationResult reconcileAllBalances(TenantContext tenantContext) {
    ...
    for (AccountNumber account : accounts) {
      ...
      if (!accountReconciliation.isBalanced()) {
        discrepanciesFound++;
        log.warn(
            "Balance discrepancy found for account {}: expected {} but calculated {}",
            account.getValue(),
            accountReconciliation.getCachedBalance(),
    ...
    log.info(
        "Balance reconciliation completed for tenant {}: {} discrepancies found, system balanced: {}",
        tenantContext.getTenantId(),
        discrepanciesFound,
        result.isSystemBalanced());
  ...
  @Transactional(readOnly = true)
  public AccountReconciliation reconcileAccountBalance(
      AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Reconciling balance for account {} and tenant {}",
    ...
    BigDecimal cachedBalance =
        accountBalanceService.getCurrentBalance(accountNumber, tenantContext);
    ...
    if (!isBalanced) {
      ...
      log.warn(
          "Balance mismatch for account {}: cached={}, calculated={}, difference={}",
          accountNumber.getValue(),
          cachedBalance,
          calculatedBalance,
          difference);
  ...
  @Transactional(readOnly = true)
  public ReconciliationResult reconcileBalancesForDateRange(
      TenantContext tenantContext, LocalDate startDate, LocalDate endDate) {
    log.info(
        "Reconciling balances for tenant {} from {} to {}",
        tenantContext.getTenantId(),
        startDate,
        endDate);
    ...
    for (AccountNumber account : accounts) {
      ...
      List<LedgerEntry> entries =
          ledgerService.getAccountEntriesByDateRange(account, tenantContext, startDate, endDate);
      ...
      if (!cachedBalance.equals(periodBalance)) {
        discrepanciesFound++;
        log.warn(
            "Period balance discrepancy for account {}: cached={}, period={}",
            account.getValue(),
            cachedBalance,
            periodBalance);
      }
    ...
    for (AccountNumber account : accounts) {
      ...
      if (!reconciliation.isBalanced()) {
        ...
        accountBalanceService.updateBalance(
            account, tenantContext, reconciliation.getCalculatedBalance());
        fixedCount++;

        log.info(
            "Fixed balance for account {}: updated from {} to {}",
            account.getValue(),
            reconciliation.getCachedBalance(),
            reconciliation.getCalculatedBalance());
      }
    }

    log.info(
        "Fixed {} balance discrepancies for tenant {}", fixedCount, tenantContext.getTenantId());
  }
  ...
  private BigDecimal calculateBalanceFromLedger(
      AccountNumber accountNumber, TenantContext tenantContext) {
  ...
  public static class ReconciliationResult {
    ...
    public boolean isSystemBalanced() {
      return systemBalanced;
    }

    public void setSystemBalanced(boolean systemBalanced) {
      this.systemBalanced = systemBalanced;
    }

    public boolean isLedgerIntegrityValid() {
      return ledgerIntegrityValid;
    }

    public void setLedgerIntegrityValid(boolean ledgerIntegrityValid) {
      this.ledgerIntegrityValid = ledgerIntegrityValid;
    }

    public List<AccountReconciliation> getAccountReconciliations() {
      return accountReconciliations;
    }

    public void setAccountReconciliations(List<AccountReconciliation> accountReconciliations) {
      this.accountReconciliations = accountReconciliations;
    }
