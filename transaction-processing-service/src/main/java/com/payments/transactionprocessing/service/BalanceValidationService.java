...
public class BalanceValidationService {
  ...
  public boolean validateDoubleEntryInvariants(Transaction transaction) {
    log.debug(
        "Validating double-entry invariants for transaction {}", transaction.getId().getValue());
    ...
    if (!totalDebits.equals(totalCredits)) {
      log.error(
          "Double-entry violation: Total debits {} != Total credits {} for transaction {}",
          totalDebits,
          totalCredits,
    ...
    if (!hasDebit || !hasCredit) {
      log.error(
          "Double-entry violation: Transaction {} must have both debit and credit entries",
          transaction.getId().getValue());
      return false;
    }
    ...
    log.debug(
        "Double-entry invariants validated for transaction {}", transaction.getId().getValue());
    return true;
  }
  ...
  public boolean validateAccountBalances(Transaction transaction) {
    log.debug("Validating account balances for transaction {}", transaction.getId().getValue());
    ...
    for (LedgerEntry entry : entries) {
      ...
      if (entry.getBalanceAfter() != null && !entry.getBalanceAfter().equals(expectedBalance)) {
        log.error(
            "Balance mismatch for account {}: expected {} but entry shows {}",
  ...
  public boolean validateAllAccountBalances(TenantContext tenantContext) {
    ...
    if (!isBalanced) {
      log.error(
          "System balance validation failed: Total balance {} != 0 for tenant {}",
          totalSystemBalance,
  ...
  public boolean validateLedgerIntegrity(TenantContext tenantContext) {
    ...
    Map<String, List<LedgerEntry>> entriesByTransaction =
        allEntries.stream().collect(Collectors.groupingBy(e -> e.getTransactionId().getValue()));

    // Validate each transaction
    for (Map.Entry<String, List<LedgerEntry>> transactionEntries :
        entriesByTransaction.entrySet()) {
      ...
      if (!totalDebits.equals(totalCredits)) {
        log.error(
            "Ledger integrity violation: Transaction {} has unbalanced entries", transactionId);
        return false;
      }
    }
  ...
  @Transactional(readOnly = true)
  public boolean validateSufficientBalance(
      AccountNumber accountNumber, TenantContext tenantContext, Money debitAmount) {
    log.debug(
        "Validating sufficient balance for account {} debit of {}",
        accountNumber.getValue(),
        debitAmount.getAmount());
    ...
    if (!hasSufficientBalance) {
      log.warn(
          "Insufficient balance: Account {} has {} but needs {} for debit",
          accountNumber.getValue(),
          currentBalance,
          debitAmount.getAmount());
    }
  ...
  private boolean validatePositiveAmounts(List<LedgerEntry> entries) {
    return entries.stream().allMatch(e -> e.getAmount().compareTo(BigDecimal.ZERO) > 0);
  }
