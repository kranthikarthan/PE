...
public class AccountBalanceService {
  ...
  @Cacheable(
      ...
      key = "#accountNumber.value + '_' + #tenantContext.tenantId")
  ...
  public void updateBalance(
      AccountNumber accountNumber, TenantContext tenantContext, BigDecimal newBalance) {
    log.debug(
        "Updating balance to {} for account {} and tenant {}",
        newBalance,
        accountNumber.getValue(),
        tenantContext.getTenantId());
  ...
  public void invalidateBalance(AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Invalidating balance cache for account {} and tenant {}",
        accountNumber.getValue(),
        tenantContext.getTenantId());
  ...
  private BigDecimal calculateBalanceFromLedger(
    ...
    var entries =
        ledgerEntryRepository.findByTenantContextAndAccountNumber(tenantContext, accountNumber);
