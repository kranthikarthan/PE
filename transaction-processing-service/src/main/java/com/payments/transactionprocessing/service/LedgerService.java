...
public class LedgerService {
  ...
  public void createLedgerEntries(Transaction transaction) {
    ...
    for (LedgerEntry ledgerEntry : transaction.getLedgerEntries()) {
      ...
      BigDecimal balanceBefore =
          accountBalanceService.getCurrentBalance(
      ...
      accountBalanceService.updateBalance(
          ledgerEntry.getAccountNumber(), transaction.getTenantContext(), balanceAfter);
    }
  ...
  public List<LedgerEntry> getAccountEntries(
      AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Getting ledger entries for account {} and tenant {}",
        accountNumber.getValue(),
        tenantContext.getTenantId());

    return ledgerEntryRepository
        .findByTenantContextAndAccountNumber(tenantContext, accountNumber)
        .stream()
        .map(LedgerEntryEntity::toDomain)
        .collect(Collectors.toList());
  }

  public List<LedgerEntry> getAccountEntriesByType(
      AccountNumber accountNumber, TenantContext tenantContext, LedgerEntryType entryType) {
    log.debug(
        "Getting {} entries for account {} and tenant {}",
        entryType,
        accountNumber.getValue(),
        tenantContext.getTenantId());

    return ledgerEntryRepository
        .findByTenantContextAndAccountNumberAndEntryType(tenantContext, accountNumber, entryType)
        .stream()
        .map(LedgerEntryEntity::toDomain)
        .collect(Collectors.toList());
  }

  public List<LedgerEntry> getAccountEntriesByDateRange(
      AccountNumber accountNumber,
      TenantContext tenantContext,
      LocalDate startDate,
      LocalDate endDate) {
    log.debug(
        "Getting ledger entries for account {} between {} and {}",
        accountNumber.getValue(),
        startDate,
        endDate);

    return ledgerEntryRepository
        .findByTenantContextAndAccountNumberAndDateRange(
  ...
  public BigDecimal getAccountBalance(AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Getting current balance for account {} and tenant {}",
        accountNumber.getValue(),
        tenantContext.getTenantId());
  ...
  public BigDecimal getAccountBalanceByType(
      AccountNumber accountNumber, TenantContext tenantContext, LedgerEntryType entryType) {
    log.debug(
        "Getting {} balance for account {} and tenant {}",
        entryType,
        accountNumber.getValue(),
        tenantContext.getTenantId());

    BigDecimal balance =
        ledgerEntryRepository.sumAmountByTenantContextAndAccountNumberAndEntryType(
  ...
  public List<LedgerEntry> getLatestAccountEntries(
      AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Getting latest entries for account {} and tenant {}",
        accountNumber.getValue(),
        tenantContext.getTenantId());

    return ledgerEntryRepository
        .findLatestByTenantContextAndAccountNumber(tenantContext, accountNumber)
        .stream()
        .map(LedgerEntryEntity::toDomain)
        .collect(Collectors.toList());
  }
  ...
  public List<AccountNumber> getAllAccountsForTenant(TenantContext tenantContext) {
    ...
    return ledgerEntryRepository.findDistinctAccountNumbersByTenantContext(tenantContext).stream()
  ...
  public List<LedgerEntry> getAllEntriesForTenant(TenantContext tenantContext) {
    ...
    return ledgerEntryRepository.findByTenantContext(tenantContext).stream()
        .map(LedgerEntryEntity::toDomain)
        .collect(Collectors.toList());
  }

  public BigDecimal getTotalDebitsForAccount(
      AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Getting total debits for account {} and tenant {}",
        accountNumber.getValue(),
        tenantContext.getTenantId());
  ...
  public BigDecimal getTotalCreditsForAccount(
      AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Getting total credits for account {} and tenant {}",
        accountNumber.getValue(),
        tenantContext.getTenantId());
  ...
  public boolean hasAccountActivity(AccountNumber accountNumber, TenantContext tenantContext) {
    log.debug(
        "Checking if account {} has activity for tenant {}",
        accountNumber.getValue(),
        tenantContext.getTenantId());

    return ledgerEntryRepository.countByTenantContextAndAccountNumber(tenantContext, accountNumber)
        > 0;
  }
