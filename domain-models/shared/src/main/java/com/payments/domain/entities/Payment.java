...
/**
 * Pure domain entity for Payment - no infrastructure dependencies Contains only business logic and
 * domain rules
 */
@Data
...
public class Payment extends AggregateRoot<PaymentId> {
  ...
  public void submitToClearing(ClearingSystemReference clearingReference) {
    ...
    addStatusChange(
        PaymentStatus.SUBMITTED_TO_CLEARING,
        "Submitted to clearing: " + clearingReference.getValue());
    addDomainEvent(
        new PaymentSubmittedToClearingEvent(
            this.paymentId, this.tenantId, this.amount, clearingReference));
  }
  ...
  public void clear(ClearingConfirmation confirmation) {
    if (this.status != PaymentStatus.SUBMITTED_TO_CLEARING) {
      throw new InvalidStateTransitionException("Cannot clear payment in status: " + this.status);
    }
    ...
    addStatusChange(
        PaymentStatus.CLEARED, "Payment cleared: " + confirmation.getClearingSystemId());
    addDomainEvent(
        new PaymentClearedEvent(this.paymentId, this.tenantId, this.amount, confirmation));
  }
  ...
  public void fail(String reason) {
    if (this.status == PaymentStatus.COMPLETED || this.status == PaymentStatus.FAILED) {
      throw new InvalidStateTransitionException("Cannot fail payment in status: " + this.status);
    }
