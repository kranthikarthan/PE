...
/**
 * Pure domain entity for Notification Preference - no infrastructure dependencies
...
public class NotificationPreferenceEntity extends AggregateRoot<PreferenceId> {
    ...
    public NotificationPreferenceEntity(PreferenceId preferenceId, TenantId tenantId, String userId,
                                        NotificationType type, NotificationChannel channel,
                                        boolean isEnabled, Map<String, Object> settings) {
        if (preferenceId == null || tenantId == null || userId == null || type == null || channel == null) {
        ...
        addDomainEvent(new PreferenceCreatedEvent(preferenceId, tenantId, userId, type, channel, this.createdAt));
    }

    // Private constructor for re-hydration from persistence (used by infrastructure layer)
    private NotificationPreferenceEntity(PreferenceId preferenceId, TenantId tenantId, String userId,
                                         NotificationType type, NotificationChannel channel,
                                         boolean isEnabled, Map<String, Object> settings,
    ...
    public void enable() {
        ...
        addDomainEvent(new PreferenceEnabledEvent(preferenceId, tenantId, userId, type, channel, this.updatedAt));
    }
    ...
    public void disable() {
        ...
        this.updatedAt = Instant.now();
        addDomainEvent(new PreferenceDisabledEvent(preferenceId, tenantId, userId, type, channel, this.updatedAt));
    }

    public void updateSettings(Map<String, Object> newSettings) {
        this.settings = newSettings;
        this.updatedAt = Instant.now();
        addDomainEvent(new PreferenceUpdatedEvent(preferenceId, tenantId, userId, type, channel, this.updatedAt));
    }

    public boolean isNotificationAllowed(NotificationType notificationType, NotificationChannel notificationChannel) {
