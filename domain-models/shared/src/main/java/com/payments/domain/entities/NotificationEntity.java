...
/**
 * Pure domain entity for Notification - no infrastructure dependencies
...
public class NotificationEntity extends AggregateRoot<NotificationId> {
    ...
    public NotificationEntity(NotificationId notificationId, TenantId tenantId, String userId,
                              NotificationType type, NotificationChannel channel,
                              String subject, String content, String recipient,
                              Map<String, Object> metadata, Instant expiresAt) {
        if (notificationId == null || tenantId == null || userId == null || type == null ||
                channel == null || subject == null || content == null || recipient == null) {
        ...
        addDomainEvent(new NotificationCreatedEvent(notificationId, tenantId, type, channel, this.createdAt));
    }

    // Private constructor for re-hydration from persistence (used by infrastructure layer)
    private NotificationEntity(NotificationId notificationId, TenantId tenantId, String userId,
                               NotificationType type, NotificationChannel channel,
                               NotificationStatus status, String subject, String content,
                               String recipient, Map<String, Object> metadata,
                               Instant createdAt, Instant sentAt, Instant deliveredAt,
                               String errorMessage, Integer retryCount, Instant expiresAt) {
        this.notificationId = notificationId;
        this.tenantId = tenantId;
        this.userId = userId;
        this.type = type;
    ...
    public void markAsSent() {
        if (this.status != NotificationStatus.PENDING) {
            throw new InvalidStateTransitionException("Notification can only be marked as sent from PENDING status.");
        }
    ...
    public void markAsDelivered() {
        if (this.status != NotificationStatus.SENT) {
            throw new InvalidStateTransitionException("Notification can only be marked as delivered from SENT status.");
        }
        this.status = NotificationStatus.DELIVERED;
        this.deliveredAt = Instant.now();
        addDomainEvent(new NotificationDeliveredEvent(notificationId, tenantId, type, channel, this.deliveredAt));
    }

    public void markAsFailed(String errorMessage) {
        if (this.status == NotificationStatus.DELIVERED || this.status == NotificationStatus.CANCELLED) {
            throw new InvalidStateTransitionException("Cannot fail a delivered or cancelled notification.");
        }
        this.status = NotificationStatus.FAILED;
        this.errorMessage = errorMessage;
        this.retryCount++;
        addDomainEvent(new NotificationFailedEvent(notificationId, tenantId, type, channel, errorMessage, Instant.now()));
    }
    ...
    public void cancel() {
        ...
        addDomainEvent(new NotificationCancelledEvent(notificationId, tenantId, type, channel, Instant.now()));
    }
