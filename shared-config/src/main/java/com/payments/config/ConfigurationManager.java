...
public class ConfigurationManager {
    ...
    public Map<String, Object> getServiceConfiguration(String serviceName) {
        ...
        config.put("database.url", getSecretValue("DATABASE_URL", "jdbc:postgresql://localhost:5432/payments"));
        ...
        config.put("kafka.bootstrap.servers", getSecretValue("KAFKA_BOOTSTRAP_SERVERS", "localhost:9092"));
    ...
    public Map<String, Object> getDatabaseConfiguration() {
        ...
        config.put("connectionTimeout", Long.parseLong(getSecretValue("DATABASE_CONNECTION_TIMEOUT", "30000")));
    ...
    public Map<String, Object> getMessagingConfiguration() {
        ...
        config.put("kafka.session.timeout.ms", Integer.parseInt(getSecretValue("KAFKA_SESSION_TIMEOUT_MS", "30000")));
        config.put("kafka.heartbeat.interval.ms", Integer.parseInt(getSecretValue("KAFKA_HEARTBEAT_INTERVAL_MS", "10000")));
        config.put("kafka.max.poll.records", Integer.parseInt(getSecretValue("KAFKA_MAX_POLL_RECORDS", "500")));
        config.put("kafka.auto.offset.reset", getSecretValue("KAFKA_AUTO_OFFSET_RESET", "earliest"));
        config.put("kafka.enable.auto.commit", Boolean.parseBoolean(getSecretValue("KAFKA_ENABLE_AUTO_COMMIT", "false")));
        config.put("kafka.enable.idempotence", Boolean.parseBoolean(getSecretValue("KAFKA_ENABLE_IDEMPOTENCE", "true")));
    ...
    public Map<String, Object> getSecurityConfiguration() {
        ...
        config.put("oauth2.client.id", getSecretValue("OAUTH2_CLIENT_ID", "payments-engine"));
        config.put("oauth2.client.secret", getSecretValue("OAUTH2_CLIENT_SECRET", "default-secret"));
        config.put("oauth2.token.uri", getSecretValue("OAUTH2_TOKEN_URI", "http://localhost:8080/oauth/token"));
        config.put("oauth2.authorization.uri", getSecretValue("OAUTH2_AUTHORIZATION_URI", "http://localhost:8080/oauth/authorize"));
        config.put("oauth2.user.info.uri", getSecretValue("OAUTH2_USER_INFO_URI", "http://localhost:8080/oauth/userinfo"));
    ...
    public Map<String, Object> getTelemetryConfiguration() {
        ...
        config.put("tracing.jaeger.endpoint", getSecretValue("TELEMETRY_JAEGER_ENDPOINT", "http://localhost:14250"));
        config.put("tracing.otlp.endpoint", getSecretValue("TELEMETRY_OTLP_ENDPOINT", "http://localhost:4317"));
        config.put("tracing.zipkin.endpoint", getSecretValue("TELEMETRY_ZIPKIN_ENDPOINT", "http://localhost:9411/api/v2/spans"));
        ...
        config.put("metrics.prometheus.endpoint", getSecretValue("TELEMETRY_PROMETHEUS_ENDPOINT", "/actuator/prometheus"));
        ...
        config.put("logging.file.path", getSecretValue("TELEMETRY_LOGGING_FILE_PATH", "logs/payments-engine.log"));
        config.put("logging.file.max.size", getSecretValue("TELEMETRY_LOGGING_FILE_MAX_SIZE", "100MB"));
        config.put("logging.file.max.history", Integer.parseInt(getSecretValue("TELEMETRY_LOGGING_FILE_MAX_HISTORY", "30")));
