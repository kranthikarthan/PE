 ...
 *
 * <p>Service for executing risk assessment rules for SAMOS clearing network: - Credit risk
...
public class SamosRiskAssessmentService {
  ...
  public SamosRiskAssessmentResponse executeRiskAssessmentRules(
    ...
    try {
      ...
      log.info(
          "SAMOS risk assessment completed: {} - Score: {}, Level: {}, Alerts: {}, Warnings: {}",
  ...
  private void executeSAMOSCreditRiskAssessment(
    ...
    if (request.getTransactionAmount() != null
        && request.getTransactionAmount().doubleValue() > 1000000) { // R1M threshold
      riskWarnings.add(
          "SAMOS credit risk: High-value transaction requires enhanced credit assessment");
    }

    // Check for credit exposure limits
    if (request.getCreditExposure() != null
        && request.getCreditExposure().doubleValue() > 5000000) { // R5M threshold
  ...
  private void executeSAMOSMarketRiskAnalysis(
      SamosAdapter adapter,
      SamosRiskAssessmentRequest request,
      List<String> appliedRules,
      List<String> riskAlerts,
    ...
    if (request.getInterestRateRisk() != null
        && request.getInterestRateRisk() > 0.05) { // 5% threshold
  ...
  private void executeSAMOSOperationalRiskEvaluation(
      SamosAdapter adapter,
      SamosRiskAssessmentRequest request,
      List<String> appliedRules,
      List<String> riskAlerts,
    ...
    if (adapter.getTimeoutSeconds() != null && adapter.getTimeoutSeconds() < 30) {
      riskWarnings.add(
          "SAMOS operational risk: Short timeout configuration may cause operational issues");
    }
  }

  /** Execute counterparty risk assessment for SAMOS transactions */
  private void executeSAMOSCounterpartyRiskAssessment(
      SamosAdapter adapter,
      SamosRiskAssessmentRequest request,
      List<String> appliedRules,
      List<String> riskAlerts,
  ...
  private void executeSAMOSSpecificRiskRules(
      SamosAdapter adapter,
      SamosRiskAssessmentRequest request,
      List<String> appliedRules,
      List<String> riskAlerts,
