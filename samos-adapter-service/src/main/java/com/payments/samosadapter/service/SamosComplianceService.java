 ...
 *
 * <p>Service for executing compliance rules for SAMOS clearing network: - AML (Anti-Money
 * Laundering) checks - KYC (Know Your Customer) validation - Regulatory reporting requirements -
 * Sanctions screening
 */
@Slf4j
@Service
...
public class SamosComplianceService {
  ...
  public SamosComplianceResponse executeComplianceRules(
    ...
    try {
      ...
      executeSAMOSKYCValidation(
          adapter, request, appliedRules, complianceErrors, complianceWarnings);

      // Rule 3: Sanctions screening for SAMOS transactions
      executeSAMOSSanctionsScreening(
          adapter, request, appliedRules, complianceErrors, complianceWarnings);

      // Rule 4: Regulatory reporting for SAMOS
      executeSAMOSRegulatoryReporting(
          adapter, request, appliedRules, complianceErrors, complianceWarnings);

      // Rule 5: SAMOS-specific compliance rules
      executeSAMOSSpecificRules(
          adapter, request, appliedRules, complianceErrors, complianceWarnings);
      ...
      log.info(
          "SAMOS compliance check completed: {} - Applied rules: {}, Errors: {}, Warnings: {}",
          isCompliant ? "COMPLIANT" : "NON-COMPLIANT",
          appliedRules.size(),
          complianceErrors.size(),
          complianceWarnings.size());
  ...
  private void executeSAMOSAMLCheck(
      SamosAdapter adapter,
      SamosComplianceRequest request,
    ...
    if (request.getAmount() != null
        && request.getAmount().doubleValue() > 1000000) { // R1M threshold
  ...
  private void executeSAMOSKYCValidation(
      SamosAdapter adapter,
      SamosComplianceRequest request,
      List<String> appliedRules,
      List<String> complianceErrors,
    ...
    if (request.getBeneficiaryAccount() == null
        || request.getBeneficiaryAccount().trim().isEmpty()) {
  ...
  private void executeSAMOSSanctionsScreening(
      SamosAdapter adapter,
      SamosComplianceRequest request,
      List<String> appliedRules,
      List<String> complianceErrors,
    ...
    if (request.getBeneficiaryName() != null && isSanctionedEntity(request.getBeneficiaryName())) {
      complianceErrors.add(
          "SAMOS sanctions screening failed: Beneficiary appears on sanctions list");
    }
  ...
  private void executeSAMOSRegulatoryReporting(
      SamosAdapter adapter,
      SamosComplianceRequest request,
      List<String> appliedRules,
      List<String> complianceErrors,
    ...
    if (request.getAmount() != null
        && request.getAmount().doubleValue() > 25000) { // R25K threshold
    ...
    if (request.getBeneficiaryCountry() != null && !"ZA".equals(request.getBeneficiaryCountry())) {
      complianceWarnings.add(
          "SAMOS regulatory reporting: Cross-border transaction requires additional reporting");
    }
  }

  /** Execute SAMOS-specific compliance rules */
  private void executeSAMOSSpecificRules(
      SamosAdapter adapter,
      SamosComplianceRequest request,
      List<String> appliedRules,
      List<String> complianceErrors,
    ...
    if (!adapter.getEncryptionEnabled()) {
      complianceWarnings.add(
          "SAMOS specific rule: Encryption not enabled for high-value transactions");
    }
  }
