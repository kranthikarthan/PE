 ...
 *
 * <p>Service for executing fraud detection rules for SAMOS clearing network: - Transaction pattern
 * analysis - Velocity checks - Geographic analysis - Behavioral analysis
 */
@Slf4j
@Service
...
public class SamosFraudDetectionService {
  ...
  public SamosFraudDetectionResponse executeFraudDetectionRules(
    ...
    try {
      ...
      log.info(
          "SAMOS fraud detection completed: {} - Score: {}, Alerts: {}, Warnings: {}",
  ...
  private void executeSAMOSVelocityCheck(
    ...
    if (request.getTransactionAmount() != null
        && request.getTransactionAmount().doubleValue() > 1000000) {
      fraudWarnings.add(
          "SAMOS velocity check: High-value transaction requires additional verification");
    }
  ...
  private void executeSAMOSGeographicAnalysis(
      SamosAdapter adapter,
      SamosFraudDetectionRequest request,
      List<String> appliedRules,
      List<String> fraudAlerts,
  ...
  private void executeSAMOSBehavioralAnalysis(
      SamosAdapter adapter,
      SamosFraudDetectionRequest request,
      List<String> appliedRules,
      List<String> fraudAlerts,
  ...
  private void executeSAMOSPatternAnalysis(
      SamosAdapter adapter,
      SamosFraudDetectionRequest request,
      List<String> appliedRules,
      List<String> fraudAlerts,
    ...
    if (request.getTransactionAmount() != null
      ...
      fraudWarnings.add(
          "SAMOS pattern analysis: Round number transaction may indicate structuring");
    }

    // Check for unusual payment references
    if (request.getPaymentReference() != null && request.getPaymentReference().length() < 5) {
  ...
  private void executeSAMOSSpecificFraudRules(
      SamosAdapter adapter,
      SamosFraudDetectionRequest request,
      List<String> appliedRules,
      List<String> fraudAlerts,
