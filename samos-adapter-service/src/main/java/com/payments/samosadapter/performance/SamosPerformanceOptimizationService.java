 ...
 *
 * <p>Advanced performance optimization service for SAMOS adapter: - Advanced caching strategies - Performance monitoring - Query optimization - Resource management
 */
@Slf4j
@Service
...
public class SamosPerformanceOptimizationService {
  ...
  public SamosPerformanceOptimizationService(MeterRegistry meterRegistry, SamosAdapterRepository samosAdapterRepository) {
    this.meterRegistry = meterRegistry;
    this.samosAdapterRepository = samosAdapterRepository;
    
    // Initialize performance counters
    this.samosCacheHitCounter = Counter.builder("samos.cache.hits.total")
        .description("Total number of SAMOS cache hits")
        .register(meterRegistry);
    
    this.samosCacheMissCounter = Counter.builder("samos.cache.misses.total")
        .description("Total number of SAMOS cache misses")
        .register(meterRegistry);
    
    this.samosQueryOptimizationCounter = Counter.builder("samos.query.optimizations.total")
        .description("Total number of SAMOS query optimizations")
        .register(meterRegistry);
    
    this.samosResourceOptimizationCounter = Counter.builder("samos.resource.optimizations.total")
        .description("Total number of SAMOS resource optimizations")
        .register(meterRegistry);
    
    // Initialize performance timers
    this.samosQueryTimer = Timer.builder("samos.query.duration")
        .description("SAMOS query execution duration")
        .register(meterRegistry);
    
    this.samosCacheTimer = Timer.builder("samos.cache.duration")
        .description("SAMOS cache operation duration")
        .register(meterRegistry);
    
    this.samosResourceTimer = Timer.builder("samos.resource.duration")
    ...
    Gauge.builder("samos.optimized.queries", this, SamosPerformanceOptimizationService::getOptimizedQueries)
  ...
  public SamosAdapter getOptimizedAdapter(String adapterId) {
    ...
    try {
      ...
      SamosAdapter adapter = samosAdapterRepository.findById(com.payments.domain.shared.ClearingAdapterId.of(adapterId))
  ...
  public List<SamosAdapter> getOptimizedAdaptersByStatus(String status) {
    ...
    try {
      ...
      List<SamosAdapter> adapters = samosAdapterRepository.findByStatus(com.payments.domain.clearing.AdapterOperationalStatus.valueOf(status));
      
      log.debug("Retrieved optimized SAMOS adapters by status: {}, count: {}", status, adapters.size());
      return adapters;
    } finally {
      sample.stop(samosQueryTimer);
    }
  ...
  public Map<String, Object> getPerformanceMetrics() {
    ...
    metrics.put("averageQueryTime", samosQueryTimer.mean(java.util.concurrent.TimeUnit.MILLISECONDS));
    metrics.put("maxQueryTime", samosQueryTimer.max(java.util.concurrent.TimeUnit.MILLISECONDS));
    
    // Resource metrics
    metrics.put("averageResourceTime", samosResourceTimer.mean(java.util.concurrent.TimeUnit.MILLISECONDS));
