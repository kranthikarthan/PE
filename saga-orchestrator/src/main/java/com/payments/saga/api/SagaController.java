...
public class SagaController {
  ...
  @Operation(
      ...
      description = "Start a new saga with the specified template")
  public ResponseEntity<SagaResponse> startSaga(
    ...
    log.info(
        "Starting saga with template {} for payment {}",
        request.getTemplateName(),
        request.getPaymentId());

    TenantContext tenantContext =
        TenantContext.of(tenantId, "Tenant", businessUnitId, "Business Unit");
  ...
  public ResponseEntity<List<SagaStepResponse>> getSagaSteps(
    ...
    return ResponseEntity.ok(steps.stream().map(SagaStepResponse::fromDomain).toList());
  }
  ...
  public ResponseEntity<List<SagaEventResponse>> getSagaEvents(
    ...
    List<com.payments.saga.domain.SagaEvent> events =
        sagaOrchestrator.getSagaEvents(SagaId.of(sagaId));

    return ResponseEntity.ok(events.stream().map(SagaEventResponse::fromDomain).toList());
  }

  /** Start compensation for a saga */
  @PostMapping("/{sagaId}/compensate")
  @Operation(
      summary = "Start saga compensation",
      description = "Start compensation for a failed saga")
  ...
  @Operation(
      summary = "Get sagas by correlation ID",
      description = "Get all sagas for a correlation ID")
