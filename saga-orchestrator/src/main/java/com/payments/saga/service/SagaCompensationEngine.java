...
public class SagaCompensationEngine {
  ...
  public void startCompensation(Saga saga) {
    ...
    for (SagaStep step : completedSteps) {
      if (step.hasCompensation()) {
        ...
        log.warn(
            "Step {} has no compensation endpoint, marking as compensated",
            step.getId().getValue());
  ...
  public void compensateStep(SagaStep step) {
    ...
    if (step.getStatus() != SagaStepStatus.COMPLETED) {
      log.warn(
          "Step {} is not completed, current status: {}",
          step.getId().getValue(),
          step.getStatus());
      return;
    }
  ...
  private Map<String, Object> executeCompensation(SagaStep step) {
    ...
    try {
      ...
      Map<String, Object> response =
          restTemplate.postForObject(url, compensationRequest, Map.class);

      log.debug(
          "Compensation for step {} completed with response: {}", step.getStepName(), response);
      return response != null ? response : Map.of("compensation", "completed");

    } catch (Exception e) {
      log.error(
          "Compensation execution failed for step {}: {}", step.getStepName(), e.getMessage(), e);
  ...
  private boolean hasCompensation(SagaStep step) {
    return step.getCompensationEndpoint() != null
        && !step.getCompensationEndpoint().trim().isEmpty();
  }
}
