...
public class PaymentValidatedEventConsumer {
  ...
  public void handlePaymentValidated(
    ...
    try {
      log.info(
          "Received PaymentValidatedEvent from topic {} (partition: {}, offset: {})",
          topic,
          partition,
          offset);
      ...
      if (isValid != null && isValid) {
        ...
        if (saga.isPresent()) {
          sagaOrchestrator.startCompensation(
              saga.get().getId(), "Validation failed: " + validationResult);
          log.info(
              "Started compensation for saga {} due to validation failure",
              saga.get().getId().getValue());
        } else {
