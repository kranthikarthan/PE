...
public interface SagaEventRepository extends JpaRepository<SagaEventEntity, String> {
  ...
  @Query(
      "SELECT e FROM SagaEventEntity e WHERE e.sagaId = :sagaId AND e.occurredAt >= :fromDate AND e.occurredAt <= :toDate ORDER BY e.occurredAt")
  ...
  @Query(
      "SELECT e FROM SagaEventEntity e WHERE e.tenantId = :tenantId AND e.businessUnitId = :businessUnitId AND e.eventType = :eventType ORDER BY e.occurredAt DESC")
  ...
  @Query(
      "SELECT COUNT(e) FROM SagaEventEntity e WHERE e.sagaId = :sagaId AND e.eventType = :eventType")
  long countBySagaIdAndEventType(
      @Param("sagaId") String sagaId, @Param("eventType") String eventType);
}
