...
public interface SagaStepRepository extends JpaRepository<SagaStepEntity, String> {
  ...
  SagaStepEntity findBySagaIdAndSequence(
      @Param("sagaId") String sagaId, @Param("sequence") Integer sequence);

  @Query(
      "SELECT s FROM SagaStepEntity s WHERE s.sagaId = :sagaId AND s.status IN :statuses ORDER BY s.sequence")
  List<SagaStepEntity> findBySagaIdAndStatusIn(
      @Param("sagaId") String sagaId, @Param("statuses") List<SagaStepStatus> statuses);

  @Query("SELECT COUNT(s) FROM SagaStepEntity s WHERE s.sagaId = :sagaId AND s.status = :status")
  long countBySagaIdAndStatus(
      @Param("sagaId") String sagaId, @Param("status") SagaStepStatus status);

  @Query(
      "SELECT s FROM SagaStepEntity s WHERE s.sagaId = :sagaId AND s.status = :status ORDER BY s.sequence DESC")
  List<SagaStepEntity> findCompletedStepsBySagaIdOrderBySequenceDesc(
      @Param("sagaId") String sagaId, @Param("status") SagaStepStatus status);

  @Query(
      "SELECT s FROM SagaStepEntity s WHERE s.tenantId = :tenantId AND s.businessUnitId = :businessUnitId AND s.status = :status")
