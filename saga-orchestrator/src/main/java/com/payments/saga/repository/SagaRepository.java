...
public interface SagaRepository extends JpaRepository<SagaEntity, String> {
    ...
    @Query("SELECT s FROM SagaEntity s WHERE s.tenantId = :tenantId AND s.businessUnitId = :businessUnitId AND s.status IN :statuses")
    ...
    List<SagaEntity> findTimedOutSagas(@Param("status") SagaStatus status, @Param("timeoutThreshold") Instant timeoutThreshold);
    
    @Query("SELECT COUNT(s) FROM SagaEntity s WHERE s.tenantId = :tenantId AND s.businessUnitId = :businessUnitId AND s.status = :status")
    ...
    @Query("SELECT s FROM SagaEntity s WHERE s.correlationId = :correlationId ORDER BY s.startedAt DESC")
    List<SagaEntity> findByCorrelationIdOrderByStartedAtDesc(@Param("correlationId") String correlationId);
