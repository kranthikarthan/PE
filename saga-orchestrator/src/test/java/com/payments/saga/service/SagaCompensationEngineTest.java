...
class SagaCompensationEngineTest {
  ...
  void setUp() {
    sagaCompensationEngine =
        new SagaCompensationEngine(restTemplate, sagaStepService, sagaEventService);
  ...
  void testStartCompensation_Success() {
    ...
    when(sagaStepService.getCompletedStepsBySagaId(any(SagaId.class))).thenReturn(completedSteps);
    ...
    verify(restTemplate, times(completedSteps.size()))
