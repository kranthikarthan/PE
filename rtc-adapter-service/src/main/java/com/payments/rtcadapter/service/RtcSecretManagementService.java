 ...
 *
 * <p>Service for managing RTC adapter secrets and secure configuration: - API keys management -
...
public class RtcSecretManagementService {
  ...
  public String getApiKey(TenantContext tenantContext) {
    String key =
        "RTC_API_KEY_" + tenantContext.getTenantId() + "_" + tenantContext.getBusinessUnitId();
  ...
  public void storeApiKey(TenantContext tenantContext, String apiKey) {
    ...
    log.info(
        "Stored RTC API key for tenant: {} and business unit: {}",
  ...
  public void storeOAuth2Credentials(
    ...
    log.info(
        "Stored RTC OAuth2 credentials for tenant: {} and business unit: {}",
        tenantContext.getTenantId(),
        tenantContext.getBusinessUnitId());
  }
  ...
  public void storeEncryptionKey(TenantContext tenantContext, String encryptionKey) {
    ...
    log.info(
        "Stored RTC encryption key for tenant: {} and business unit: {}",
        tenantContext.getTenantId(),
        tenantContext.getBusinessUnitId());
  }
  ...
  public String getEndpointUrl(TenantContext tenantContext) {
    String key =
        "RTC_ENDPOINT_URL_" + tenantContext.getTenantId() + "_" + tenantContext.getBusinessUnitId();
  ...
  public void storeEndpointUrl(TenantContext tenantContext, String endpointUrl) {
    ...
    log.info(
        "Stored RTC endpoint URL for tenant: {} and business unit: {}",
        tenantContext.getTenantId(),
        tenantContext.getBusinessUnitId());
  }
  ...
  public String getCertificate(TenantContext tenantContext) {
    String key =
        "RTC_CERTIFICATE_" + tenantContext.getTenantId() + "_" + tenantContext.getBusinessUnitId();
    return secretManager.getEncryptedSecret(key);
  }
  ...
  public void storeCertificate(TenantContext tenantContext, String certificate) {
    ...
    log.info(
        "Stored RTC certificate for tenant: {} and business unit: {}",
        tenantContext.getTenantId(),
        tenantContext.getBusinessUnitId());
  }
  ...
  public void removeAllSecrets(TenantContext tenantContext) {
    ...
    log.info(
        "Removed all RTC secrets for tenant: {} and business unit: {}",
        tenantContext.getTenantId(),
        tenantContext.getBusinessUnitId());
  }
}
