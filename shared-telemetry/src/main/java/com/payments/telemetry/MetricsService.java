...
public class MetricsService {
  ...
  public <T> T recordStepExecution(String stepName, String stepType, Supplier<T> operation) {
    return recordTiming(
        ...
        Map.of("step.name", stepName, "step.type", stepType),
        operation);
  }

  /** Record external service call timing */
  public <T> T recordExternalServiceCall(
      String serviceName, String operation, Supplier<T> operationCode) {
    return recordTiming(
        "external.service.call.duration",
        Map.of("service.name", serviceName, "operation", operation),
        operationCode);
  }
  ...
  public <T> T recordCacheOperation(String operation, String cacheName, Supplier<T> operationCode) {
    return recordTiming(
        ...
        Map.of("operation", operation, "cache.name", cacheName),
        operationCode);
  }
  ...
  public <T> T recordKafkaOperation(String operation, String topic, Supplier<T> operationCode) {
    return recordTiming(
        "kafka.operation.duration", Map.of("operation", operation, "topic", topic), operationCode);
  }
