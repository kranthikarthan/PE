 ...
 */
@Repository
public interface BankservAfricaEftMessageRepository extends JpaRepository<BankservAfricaEftMessage, ClearingMessageId> {
     ...
     */
    @Query("SELECT m FROM BankservAfricaEftMessage m WHERE m.bankservafricaAdapterId = :adapterId AND m.status = :status")
